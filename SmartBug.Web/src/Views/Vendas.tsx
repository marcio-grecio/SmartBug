...
import { addVenda, getAllVendas, getVenda, UpdateVenda } from '../Services/VendaService';
...
const Vendas: React.FC = () => {
  ...
  const handleButtonClick = async (id: any) => {
    try {
      const vendaData = await getVenda(id);

      setFormData({
        id: vendaData.id, 
        data: vendaData.data,
        quantidade: vendaData.quantidade,
        empreendimentoId: vendaData.empreendimentoId,
      });
  ...
  const getPropostaData = useCallback(async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setVendas(formatterData);
        setFilteredVendas(formatterData);
      }
  ...
  const handleSubmit = async () => {
    try {
      const serviceFunction = formData.id ? UpdateVenda : addVenda;
  ...
  useEffect(() => {
    const results = vendas.filter((search) =>
  ...
  return (
    <section
      ...
      {loading && <Loading fullscreen text="Carregando Vendas Efetivadas..." />}
      {!loading && (
        <>
