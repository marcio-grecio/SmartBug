...
import PropostaForm from '../Components/Proposta/Index';
import DataTable, { createTheme } from 'react-data-table-component';
import { useCallback, useContext, useEffect, useMemo, useState } from 'react';
import { addProposta, getAllPropostas, getProposta, UpdateProposta } from '../Services/PropostaService';
...
const Proposta: React.FC = () => {
  ...
  const handleButtonClick = async (id: any) => {
    try {
      const propostaData = await getProposta(id);

      setFormData({
        id: propostaData.id, 
        data: propostaData.data,
        quantidade: propostaData.quantidade,
        empreendimentoId: propostaData.empreendimentoId,
      });
  ...
  const getPropostaData = useCallback(async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setPropostas(formatterData);
        setFilteredPropostas(formatterData);
      }
  ...
  const handleSubmit = async () => {
    try {
      const serviceFunction = formData.id ? UpdateProposta : addProposta;
  ...
  useEffect(() => {
    const results = propostas.filter((search) =>
            ...
            <div className="row" style={{ marginTop: 0 }}>
              <div className="col-md-9 mb-2 mt-3">
                <div className="ml-4 mb-3">
                  <Button color='#28C76F' text='Novo Cliente / Proposta' onClick={toggleModal} disabled={false} height={32} width={200} fontWeight={'600'} fontFamily='nunito' icon={Plus}/>
                </div>
              </div>
